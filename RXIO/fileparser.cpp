...
void FileParser::read_event_list(QList<RPGEventCommand*> *event_command_list)
{
    ...
    for (int n = 0; n < num_commands; n++)
    {
        ...
        event_command_list->append(current_event_command);
        //current_event_command->debug();
    }
}

void FileParser::write_event_list(QList<RPGEventCommand *> *event_command_list)
{
    this->write_array(event_command_list->length());
    for (int k = 0; k < event_command_list->length(); k++)
    {
        this->write_object("RPG::EventCommand", 3);
        RPGEventCommand *current_command = event_command_list->at(k);
