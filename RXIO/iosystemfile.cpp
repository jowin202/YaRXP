...
IOSystemFile::IOSystemFile(QString path, RPGSettings *settings)
{
    ...
    for (int i = 0; i < num_params; i++)
    {
        ...
        settings->param_order.append(current_symbol);
...
void IOSystemFile::write_to_file(QString path, RPGSettings *settings)
{
    ...
    for (int i = 0; i < settings->param_order.length(); i++)
    {
        QString current_symbol = settings->param_order.at(i);
        this->write_symbol_or_link(current_symbol);

        if (current_symbol == "@cancel_se")
        ...
        else if (current_symbol == "@variables")
        {
